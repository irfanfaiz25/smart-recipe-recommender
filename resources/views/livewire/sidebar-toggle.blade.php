        ...
        <div class="mt-10 flex flex-col gap-2 relative text-gray-800 dark:text-gray-50">
            @foreach ($sidebarMenu as $menu)
                <a href="{{ route($menu['route']) }}" wire:navigate
                    class="group flex items-center text-sm h-11 gap-3.5 font-medium p-2 pl-5 hover:bg-[#f2f2f2] dark:hover:bg-[#252525] hover:text-primary dark:hover:text-primary rounded-md {{ request()->is($menu['request']) ? 'bg-[#f2f2f2] dark:bg-[#252525] text-primary' : 'text-gray-800 dark:text-gray-50' }}">
