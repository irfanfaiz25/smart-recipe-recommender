        ...
        <div x-data="{
                    ...
                    <template x-if="activeSubmenu === 'profile'">
                        <div class="space-y-1">
                            ...
                            <form method="POST" action="{{ route('auth.logout') }}"
                                x-on:submit.prevent="closeSubmenu(); $el.submit();">
                                @csrf
                                <button type="submit"
            ...
            <div class="flex justify-around items-center py-2 bg-white relative z-30">
                ...
                <button @click="toggleSubmenu('recipes')" class="flex flex-col items-center p-2 min-w-0 relative">
                    ...
                    <!-- Active indicator -->
                    <div x-show="activeSubmenu === 'recipes'" class="absolute -top-1 w-1 h-1 bg-secondary rounded-full">
                    </div>
                </button>
                ...
                <button @click="toggleSubmenu('profile')" class="flex flex-col items-center p-2 min-w-0 relative">
                    <div class="w-6 h-6 flex items-center justify-center mb-1">
                        @if (Auth::user()->avatar)
                            <img class="w-5 h-5 rounded-full object-cover" src="{{ Auth::user()->avatar }}"
                                alt="Profile">
                        @else
                    ...
                    <div x-show="activeSubmenu === 'profile'" class="absolute -top-1 w-1 h-1 bg-secondary rounded-full">
                    </div>
                </button>
            </div>
        </div>
    @else
        ...
        <div class="flex justify-around items-center py-2 bg-white relative z-30">
            ...
            <a href="{{ route('login') }}" wire:navigate class="flex flex-col items-center p-2 min-w-0">
                ...
                <span
                    class="text-xs {{ request()->is('login') ? 'text-secondary font-medium' : 'text-gray-500' }} truncate">
                    Masuk atau Daftar
                </span>
            </a>
        </div>
